...
if True:
    ...
    for ageBin in range(nAgeBins) :
        ...
        axArr[row,column].text(47,33,'Bin '+str(ageBin)+' log(t)='+str(logAgeArr[ageBin]))
...
if False:
    ...
    for ageBin in range(nBinsScheme) :
         ...
         axArr[row,column].text(47,33,'Bin '+str(ageBin)+' log(t)='+str(logAgeArr[ageBin]))
         axArr[row,column].plot((np.asarray(objListRA)-cellCentersRA[0])/(0.5*raInc),(np.asarray(objListDec)-cellCentersDec[0])/(0.5*decInc),'r.',ms=2.0)
